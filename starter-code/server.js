...
const app = express();
const conString = 'postgres://postgres:1357@localhost:5432/kilovolt';// TODO: Don't forget to set your own conString
...
app.get('/articles', function(request, response) {
  ...
  client.query(`
    ...
    FROM authors
    JOIN articles ON
    authors.author_id = articles.author_id
    `)
  ...
  app.post('/articles', function(request, response) {
    client.query(
      `
      INSERT INTO
      authors(author, "authorUrl", author_id)
      VALUES ($1, $2, $3)
      ON CONFLICT DO NOTHING
      `

      , // TODONE: Write a SQL query to insert a new author, ON CONFLICT DO NOTHING
      [request.body.author, request.body.authorUrl], // TODONE: Add the author and "authorUrl" as data for the SQL query
      function(err) {
    ...
    function queryTwo() {
      client.query(
        ...
        WHERE authors = $1
        `
        , // TODONE: Write a SQL query to retrieve the author_id from the authors table for the new artic
        [request.body.author], // TODONE: Add the author name as data for the SQL query
        function(err, result) {
          if (err) console.error(err)
      ...
      client.query(
        `
          ...
          WHERE
           article_id = $1;
          `,
        [request.body_author_id, request.body_title, request.body.category, request.body.publishedOn, request.body.body]
      )
    })
    .then(function() {
